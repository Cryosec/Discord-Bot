...
async def scam_check_embed(self, message, filtered_url):
    ...
    embed.add_field(name = 'Join date', value = message.author.joined_at.strftime(TIME_FORMAT))
    embed.add_field(name = 'Creation date', value = message.author.created_at.strftime(TIME_FORMAT))
...
async def check_msg_link(self, message):
    ...
    if 'discord.com/channels/' in message.content:
        ...
        embed.add_field(name = 'Time', value = msg.created_at.strftime(TIME_FORMAT) + ' UTC')
        embed.set_footer(text=config.FOOTER)
